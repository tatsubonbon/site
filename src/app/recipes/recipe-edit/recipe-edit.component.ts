...
export class RecipeEditComponent implements OnInit {
  ...
  private initForm() {
    ...
    let recipeIngeredient = new UntypedFormArray([]);
    ...
    if (this.editMode) {
      ...
      if (recipe['ingredients']) {
        for (let ingredient of recipe.ingredients) {
          recipeIngeredient.push(new UntypedFormGroup({
    ...
    this.recipeForm = new UntypedFormGroup({
      ...
      'description': new UntypedFormControl(recipeDescription),
      'ingredients': recipeIngeredient
    });
  }
}
